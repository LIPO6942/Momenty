...
import { Globe, Map, Compass, PlusCircle, BookText, Smile, Image as ImageIcon, Mic } from "lucide-react";
...
const addActions = [
    { label: "Note", icon: BookText, color: "text-purple-700", component: AddNoteDialog },
    { label: "Audio", icon: Mic, color: "text-orange-700", component: null },
    { label: "Photo", icon: ImageIcon, color: "text-blue-700", component: AddPhotoDialog },
      ...
      <nav className="container relative flex h-16 items-center justify-between">
        ...
        <Sheet>
            <SheetTrigger asChild>
                 <Button size="icon" className="h-16 w-16 rounded-full shadow-lg absolute -top-8 left-1/2 -translate-x-1/2 bg-accent hover:bg-accent/90">
                    <PlusCircle className="h-8 w-8 text-accent-foreground" />
                </Button>
            </SheetTrigger>
            <SheetContent side="bottom" className="rounded-t-2xl">
                <SheetHeader className="mb-4">
                    <SheetTitle className="text-center">Ajouter un instant</SheetTitle>
                </SheetHeader>
