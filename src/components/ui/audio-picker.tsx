...
export function AudioPicker({ value, onChange }: AudioPickerProps) {
  ...
  const handleManualSearch = async () => {
    ...
    try {
      ...
      if (combined.length === 0) {
        toast({ variant: "destructive", title: "Aucun rÃ©sultat", description: "Essayez d'autres mots clÃ©s comme 'piano', 'mer' ou 'vent'." });
      }
    } catch (err) {
  ...
  const startRecording = async () => {
    try {
      ...
      toast({ variant: "destructive", title: "AccÃ¨s micro refusÃ©", description: "Veuillez autoriser l'accÃ¨s au micro pour enregistrer." });
    }
  };
  ...
  const handleUpload = async (blob: Blob) => {
    ...
    try {
      ...
      toast({ title: "Enregistrement vocal sauvegardÃ© !" });
    } catch (error) {
  ...
  const togglePreview = (url: string) => {
    if (activeAudio === url && isPlaying) {
      ...
      audio.play().catch(() => {
        toast({ variant: "destructive", title: "Lecture bloquÃ©e", description: "Appuyez Ã  nouveau pour lire." });
      });
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Studio Sonore Library Modal - always mounted outside the value conditional */}
