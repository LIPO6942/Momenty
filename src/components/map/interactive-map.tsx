...
interface InteractiveMapProps {
  ...
  onMarkerClick?: (location: LocationWithCoords) => void;
}

export default function InteractiveMap({ locations, focusedLocation, showPolyline = false, isNumbered = false, travelSegments = [], onMarkerClick }: InteractiveMapProps) {
    ...
    locations.forEach((location, index) => {
      ...
      const popupContent = isNumbered
        ...
        : `<div>
            ...
            ${location.count > 0 ? `${location.count} instant(s)` : 'Lieu visité'}<br/>
            <small style="color: #666;">Cliquer pour voir les détails →</small>
          </div>`;
